 ...
 */
package org.apache.lucene.testframework.util;
...
public final class RunListenerPrintReproduceInfo extends RunListener {
  ...
  private static void printDebuggingInformation() {
    if (LuceneTestCase.classEnvRule != null && LuceneTestCase.classEnvRule.isInitialized()) {
      System.err.println("NOTE: test params are: codec=" + LuceneTestCase.classEnvRule.codec +
          ", sim=" + LuceneTestCase.classEnvRule.similarity +
          ", locale=" + LuceneTestCase.classEnvRule.locale.toLanguageTag() +
          ", timezone=" + (LuceneTestCase.classEnvRule.timeZone == null ? "(null)" : LuceneTestCase.classEnvRule.timeZone.getID()));
    }
  ...
  private void reportAdditionalFailureInfo(final String testName) {
    ...
    if (LuceneTestCase.RANDOM_MULTIPLIER > 1) addVmOpt(b, "tests.multiplier", LuceneTestCase.RANDOM_MULTIPLIER);
    if (LuceneTestCase.TEST_NIGHTLY) addVmOpt(b, LuceneTestCase.SYSPROP_NIGHTLY, LuceneTestCase.TEST_NIGHTLY);
    if (LuceneTestCase.TEST_WEEKLY) addVmOpt(b, LuceneTestCase.SYSPROP_WEEKLY, LuceneTestCase.TEST_WEEKLY);
    if (LuceneTestCase.TEST_SLOW) addVmOpt(b, LuceneTestCase.SYSPROP_SLOW, LuceneTestCase.TEST_SLOW);
    if (LuceneTestCase.TEST_MONSTER) addVmOpt(b, LuceneTestCase.SYSPROP_MONSTER, LuceneTestCase.TEST_MONSTER);
    if (LuceneTestCase.TEST_AWAITSFIX) addVmOpt(b, LuceneTestCase.SYSPROP_AWAITSFIX, LuceneTestCase.TEST_AWAITSFIX);
    if (LuceneTestCase.TEST_BADAPPLES) addVmOpt(b, LuceneTestCase.SYSPROP_BADAPPLES, LuceneTestCase.TEST_BADAPPLES);

    // Codec, postings, directories.
    if (!LuceneTestCase.TEST_CODEC.equals("random")) addVmOpt(b, "tests.codec", LuceneTestCase.TEST_CODEC);
    if (!LuceneTestCase.TEST_POSTINGSFORMAT.equals("random")) addVmOpt(b, "tests.postingsformat", LuceneTestCase.TEST_POSTINGSFORMAT);
    if (!LuceneTestCase.TEST_DOCVALUESFORMAT.equals("random")) addVmOpt(b, "tests.docvaluesformat", LuceneTestCase.TEST_DOCVALUESFORMAT);
    if (!LuceneTestCase.TEST_DIRECTORY.equals("random")) addVmOpt(b, "tests.directory", LuceneTestCase.TEST_DIRECTORY);

    // Environment.
    if (!LuceneTestCase.TEST_LINE_DOCS_FILE.equals(LuceneTestCase.DEFAULT_LINE_DOCS_FILE)) addVmOpt(b, "tests.linedocsfile", LuceneTestCase.TEST_LINE_DOCS_FILE);
    if (LuceneTestCase.classEnvRule != null && LuceneTestCase.classEnvRule.isInitialized()) {
      addVmOpt(b, "tests.locale", LuceneTestCase.classEnvRule.locale.toLanguageTag());
      if (LuceneTestCase.classEnvRule.timeZone != null) {
        addVmOpt(b, "tests.timezone", LuceneTestCase.classEnvRule.timeZone.getID());
      }
    }
