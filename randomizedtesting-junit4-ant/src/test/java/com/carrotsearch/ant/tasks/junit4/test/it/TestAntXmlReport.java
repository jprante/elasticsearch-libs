...
public class TestAntXmlReport extends JUnit4XmlTestBase {
  ...
  public void antxml() throws Exception {
    ...
    String s = TestBeforeClassError.class.getName() + ".xml";
    Assert.assertTrue(
        new File(getProject().getBaseDir(), "ant-xmls/TEST-" + s).length() > 0);

    Assert.assertTrue(
        new File(getProject().getBaseDir(), "ant-xmls/TEST-com.carrotsearch.ant.tasks.junit4.test.donotexecute.replication.TestSuiteReplicated-2.xml").length() > 0);
